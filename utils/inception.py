...
class InceptionModel(nn.Module):
    ...
    def __init__(self, num_blocks: int, in_channels: int, out_channels: Union[List[int], int],
                 ...
                 num_pred_classes: int = 1,config = None) -> None:
        super().__init__()
...
#                            kernel_size=kernel_sizes[i],config=config) for i in range(num_blocks)
#         ])
        i = 0
        ...
        if residual:
            self.residual = nn.Sequential(*[
                Conv1dSamePadding(in_channels=in_channels, out_channels=out_channels,
                                  kernel_size=1, stride=stride, bias=False, bit=bit, config=config),
